 ...
 */
/** Contenido de Mi presupuesto; `embedded` = se muestra como pestaña de "Presupuesto y ranking". */
export function MiPresupuestoPanel({
  embedded = false,
  anio: anioProp,
  mes: mesProp,
}: {
  ...
  const loadData = useCallback(async () => {
    ...
    // Un mes retail puede tocar dos meses calendario: se leen los horarios de ambos.
  ...
  const semanas = useMemo(() => {
    ...
    return agruparMetasPorSemana(distribucion, anio, mes, periodo);
  }, [distribucion, anio, mes, periodo]);

  const hoy = new Date();
  const fechaHoy = ymd(hoy);
  const esMesActual = fechaHoy >= periodo.inicio && fechaHoy <= periodo.fin;
  ...
  return (
    <ShellCond embedded={embedded} persona={persona}>
      <div className={"mx-auto max-w-[900px] w-full " + (embedded ? "py-2" : "px-6 py-7")}>
        {!embedded && (
        <div className="mb-5">
          ...
          <>
            ...
            <div className="bg-panel border border-line rounded-[10px] p-4">
              ...
              <div className="grid grid-cols-7 gap-1">
                {(() => {
                  const primerWD = parseYmd(periodo.inicio).getDay();
                  ...
                  for (const dm of diasDelMes) {
                    const fecha = dm.fecha;
                    const md = distribucion.diaria.get(fecha);
                    const esHoy = dm.fecha === fechaHoy;
